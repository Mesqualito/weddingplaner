...
public interface AllowControlRepository extends JpaRepository<AllowControl, Long> {
    @Query("select distinct allow_control from AllowControl allow_control left join fetch allow_control.controlledGroup")
    List<AllowControl> findAllWithEagerRelationships();

    @Query("select allow_control from AllowControl allow_control left join fetch allow_control.controlledGroup where allow_control.id =:id")
